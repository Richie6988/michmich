...
export default function CagnottePage() {
  ...
  const { cagnottes, payContribution, initCagnotte, activeTrip, currentUser, trips } = useAppStore();
      ...
      <div className="px-4 py-6">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-barry-grey mt-1.5 max-w-xs mx-auto">
            Chacun pre-paie sa part. Tu reserves tout en un clic.
          </p>
        </div>
        ...
        {isAdmin ? (
          <div className="bg-white rounded-2xl p-5 border border-gray-100">
            ...
            <p className="text-xs text-barry-grey mb-4">
              Soit ~{Math.ceil(setupAmount / Math.max(trip?.participants.length || 1, 1))} EUR par personne
            </p>
            <button
              onClick={() => initCagnotte(id as string, setupAmount)}
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-center">
            <p className="text-sm text-amber-800">
              Seul l'organisateur peut lancer une cagnotte.
            </p>
          </div>
        )}
      </div>
    ...
    <div className="px-4 py-4">
      ...
      <div className="bg-gradient-to-br from-pink-500 to-rose-600 rounded-3xl p-5 text-white mb-4 shadow-lg shadow-pink-500/20">
        <div className="flex items-center justify-between mb-4">
          <span className="text-xs uppercase tracking-wider font-semibold text-pink-100">Cagnotte du groupe</span>
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-pink-100 mt-1">
            {remaining > 0 ? `${remaining} EUR restants` : 'Cagnotte complete !'}
          </p>
        </div>
      ...
      {myContribution && myContribution.status === 'pending' && (
        <div className="bg-white rounded-2xl p-4 mb-4 border-2 border-barry-coral/30">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-xs text-barry-grey">Ta part</p>
          ...
          <p className="text-[11px] text-barry-grey text-center mt-2">
            Stripe Connect (mode demo) - aucun debit reel
          </p>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl p-4 mb-4 border border-gray-100">
        <h3 className="font-semibold text-barry-black mb-3 flex items-center justify-between">
          ...
          <span className="text-xs text-barry-grey font-normal">
            {cagnotte.contributions.filter(c => c.status === 'paid').length}/{cagnotte.contributions.length} payes
          </span>
        </h3>
            ...
            <div key={c.id} className="flex items-center justify-between py-1.5">
              ...
              {c.status === 'paid' ? (
                <div className="flex items-center gap-1 text-emerald-700 bg-emerald-50 px-2 py-1 rounded-full">
                  ...
                  <span className="text-[10px] font-bold uppercase">Paye</span>
                </div>
              ) : (
        ...
        <div className={`rounded-2xl p-4 border-2 transition-all ${
          ...
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1">
              <p className="font-semibold text-barry-black">Reservation 1-clic</p>
              <p className="text-xs text-barry-grey mt-0.5">
                {allPaid
                  ? 'Tout est paye ! Tu peux reserver transport + venue + activite en un clic.'
                  : 'Disponible une fois que tout le monde a paye.'}
              </p>
            </div>
          </div>
          <button
